 ...
 * @LastEditTime : 2020-01-03 18:09:43
 * @LastEditors  : liuqiyu
...
<template>
  <el-form-item :label="item.label"
    ...
    <template v-else-if="item.type === 'checkbox'">
      <el-checkbox v-if="item.checkAll"
                   :indeterminate="isIndeterminate"
                   v-model="checkAll"
                   @change="handleCheckAllChange">全选</el-checkbox>
      <el-checkbox-group :class="item.class"
                         :style="item.style"
                         :value="value"
                         v-bind="item.props"
                         @input="bindChange"
    ...
    <file-add v-if="item.type === 'fileAdd'"
              :group-id.sync="value[0]"
              :file-id.sync="value[1]"
              v-bind="item.props">
      <template v-if="item.slot"
                slot="right">
        <el-link :href="item.slot.href"
                 type="primary"
                 @click.native.stop>{{ item.slot.label }}</el-link>
      </template>
    </file-add>
  </el-form-item>
</template>
...
export default {
  ...
  methods: {
    ...
    handleCheckAllChange (val) {
      const options = this.item.options.map(item => {
        return item.value
      })
      console.log(options)
      this.$emit('input', val ? options : [])
